...
public static class TextureExtension
{
    ...
    public static void FloodFillArea(this Texture2D aTex, int aX, int aY, Color aFillColor, int floodWidth)
    {
        int w = aTex.width;
        int h = aTex.height;
        Color[] colors = aTex.GetPixels();
        Color refCol = colors[aX + aY * 1];
        ...
        while (nodes.Count > 0)
        {
            ...
            for (int i = current.x; i < w; i++)
            {
                ...
                if (C != refCol || C == aFillColor) // points that are not colored
                    break;
                colors[i + current.y * w] = aFillColor;
                if (i > floodWidth) colors[i + current.y * w] = Color.white;
                if (current.y + 1 < h)
                {
    ...
    public static void FillAreaByWidth(this Texture2D aTex, int aX, int aY, Color aFillColor, int fillWidth)
    {
        int w = aTex.width;
        int h = aTex.height;
        Color[] colors = aTex.GetPixels();
        ...
        while (nodes.Count > 0)
        {
            ...
            {
                Color C = colors[i + current.y * w];
                if (C == aFillColor) // points that are not colored
                    break;
                colors[i + current.y * w] = aFillColor;
                if (current.y + 1 < h)
                {
